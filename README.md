...
- There is no automated regression suite for the catalogue evaluation cases.
...
The plugin depends on WordPress, WooCommerce, Advanced Custom Fields (`get_fields`), the site's `batch` post type, and the category/effect taxonomies. Its client script and stylesheet are in `assets/js/es-smart-search.js` and `assets/css/es-smart-search.css`; server-side indexing and ranking live in `es-smart-search.php`.

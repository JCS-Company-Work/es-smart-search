export class UrlService {
  ...
  writeUrlState() {
    ...
    if (hasSearchableQuery) {
      parts.push(`textsearch=${encodeURIComponent(this.app.state.query)}`);
    } else if (this.app.state.query && !hasFilters && !urlHasQuery) {
      return;
    }

    // Add each filter group and its values to the parts array for the URL hash
    Object.entries(this.app.state.filters).forEach(([group, values]) => {
      if (values.length) {
        parts.push(
    ...
    parts.push(`page=${this.app.state.page}`);
